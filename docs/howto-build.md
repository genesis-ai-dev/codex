...
```
docker run -ti --volume=<local codex source>:/root/codex --name=codex-build-agent codex/codex-linux-build-agent:bionic-x64 bash
```
like
```
docker run -ti --volume=$(pwd):/root/codex --name=codex-build-agent codex/codex-linux-build-agent:bionic-x64 bash
```
...
Firstly, create the container with:
```
docker run -ti --volume=<local codex source>:/root/codex --name=codex-build-agent codex/codex-linux-build-agent:bionic-armhf bash
```
like
```
docker run -ti --volume=$(pwd):/root/codex --name=codex-build-agent codex/codex-linux-build-agent:bionic-armhf bash
```
