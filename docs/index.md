...
- [Extensions + Marketplace](#extensions-marketplace)
  ...
  - [How to use the Codex Marketplace](#howto-vscode-marketplace)
  - [How to self host your own Codex Marketplace](#howto-selfhost-marketplace)
...
Being a vscode based editor, Codex gets additional features by installing Codex extensions.
Unfortunately, as Microsoft [prohibits usages of the Microsoft marketplace by any other products](https://github.com/microsoft/vscode/issues/31168) or redistribution of `.vsix` files from it, in order to use Codex extensions in non-Microsoft products those need to be installed differently.

By default, the `product.json` file is set up to use [open-vsx.org](https://open-vsx.org/) as extension gallery, which has an [adapter](https://github.com/eclipse/openvsx/wiki/Using-Open-VSX-in-VS-Code) to the Marketplace API used by Codex. Since that is a rather new project, you will likely miss some extensions you know from the Codex Marketplace. You have the following options to obtain such missing extensions:

* Ask the extension maintainers to publish to [open-vsx.org](https://open-vsx.org/) in addition to the Codex Marketplace. The publishing process is documented in the [Open VSX Wiki](https://github.com/eclipse/openvsx/wiki/Publishing-Extensions).
...
These examples use the URLs for Microsoft's Codex Marketplace, see [below](#howto-vscode-marketplace) for more information on that.
...
* [Open VSX](https://github.com/eclipse/openvsx) eclipse open-source project
  While the public instance which is run by the Eclipse Foundation is the pre-set endpoint in Codex, you can host your own instance.
    ...
    > Open VSX is a [vendor-neutral](https://projects.eclipse.org/projects/ecd.openvsx) open-source alternative to the [Visual Studio Marketplace](https://marketplace.visualstudio.com/vscode). It provides a server application that manages [Codex extensions](https://code.visualstudio.com/api) in a database, a web application similar to the Codex Marketplace, and a command-line tool for publishing extensions similar to [vsce](https://code.visualstudio.com/api/working-with-extensions/publishing-extension#vsce).
...
### <a id="howto-vscode-marketplace"></a>How to use the Codex Marketplace
